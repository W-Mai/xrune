...
**附魔**是部件后附的一段方括号表达式串，位置坐落在属性与 body 之间：
...
无论那一槽省略与否，符文师收到的都是一个 `Vec`：省了就是空 vec。

## 用法形态

附魔故意不带类型，这正是它的用处：让用户把**任何东西**挂到节点上，无需在语法里给它单独留位。一个典型的 ECS 风符文师会把每个附魔表达式转成一个组件，插到 spawn 出来的实体上：
